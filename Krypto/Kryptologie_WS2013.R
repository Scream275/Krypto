...
ReadFunctions<-function() {
	source("D:/Programme/R/Skripte/Read_in_all_spectrum_files_of_a_directory.R")
	source("D:/Programme/R/Skripte/Read_in_the_data_of_a_file_with_sep.R")
}

# Main program
ReadFunctions()
directory="D:/Eigene Dateien/workspace/Haeufigkeitsanalyse"
setwd(directory)
pattern<-"haeufigkeitsanalyse_edited_[0-9]*"
...
png.file<-paste("haeufigkeitsanalyse_edited", ".png")
png(file=png.file,height=1000, width=1400, res=100)
...
plot(dataSorted$buchstabe, dataSorted$haeufigkeit, type="p", col="blue", lwd=2, 
	...
	cex.axis=1, font.axis=2, font.lab=2, lty=1, ylab="Haeufigkeit", ylim=c(0,60))
